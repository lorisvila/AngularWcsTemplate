...
export class DataModule {
  ...
  constructor(mainClass: App) {
    ...
    this.dbEvent.on('askConnect', () => {
      this.connectToServer()
    })

    this.dbEvent.emit('askConnect')
  ...
  async connectToServer() {
    try {
      ...
      let seconds = 5
      this.logger.warn(`Failed to connect to server, trying again in ${seconds}`)
      setTimeout(() => {
        this.dbEvent.emit('askConnect')
      },seconds * 1000)
    }
  ...
  async sendSQLquery(sqlQuery: string) {
    if (!this.dbClient) {
      throw new API_Error('API_INTERN_ERROR', "Le serveur n'est pas connecté à la BDD")
    }
    return this.dbClient.query(sqlQuery)
  }

  async createEntry(newObject: any, tableName: TableName, skipCheck: boolean = false): Promise<void> {
    ...
    `
      ...
      try {
        ...
        this.handleDataBaseErrror(error, checkQuery)
      }
    ...
    try {
      ...
      this.handleDataBaseErrror(error, addQuery)
    }
  ...
  async editEntry(oldObject: any, newObject: any, tableName: TableName): Promise<void> {
    ...
    try {
      ...
      this.handleDataBaseErrror(error, replaceQuery)
    }
  ...
  async deleteEntry(object: any, tableName: TableName): Promise<void> {
    ...
    try {
      ...
      this.handleDataBaseErrror(error, deleteQuery)
    }
  ...
  getAndCheckKeysAndValues(tableName: string, newObject: any, oldObject?: any) {
    ...
    keys.forEach((key: keyof typeof newObject) => {
      try {
        values = [...values, typeof newObject[key] == "string" ? newObject[key].replace(/'/g, "''") as string : JSON.stringify(newObject[key]).replace(/'/g, "''")]
      } catch (error) {
  ...
  handleDataBaseErrror(error: any, queryString: string) {
